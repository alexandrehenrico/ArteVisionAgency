...
export const gerarPDFOrcamento = (orcamento: Orcamento) => {
  ...
  const addHeader = () => {
    ...
    pdf.text('Arte Vision Agencyma@gmail.com • (99) 98468-0391', margin, yPosition + 20)
  ...
  const addFooter = (pageNum: number) => {
    ...
    pdf.setFont('helvetica', 'normal')
    pdf.setFontSize(8)
    pdf.text('Arte Vision Agency - Desenvolvedora de Software', margin, footerY + 8)
    pdf.text(`Gerado em ${new Date().toLocaleString('pt-BR')}`, margin, footerY + 15)
    pdf.text('Arte Vision Agencyma.com', pageWidth - 40, footerY + 8)
