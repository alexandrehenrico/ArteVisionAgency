...
export function UserHeader({ showWelcome = true, variant = 'default', className }: UserHeaderProps) {
    ...
    return (
      <div className={`flex items-center gap-3 ${className || ''}`}>
        <Avatar className="h-8 w-8">
          ...
          <AvatarFallback className="text-sm font-semibold bg-gradient-to-br from-blue-800 to-black text-white">
            {getInitials(displayName)}
          </AvatarFallback>
        </Avatar>
        <div className="flex flex-col flex-1 min-w-0">
          <span className="text-sm font-medium text-blue-900">{firstName}</span>
  ...
  return (
    <div className={`relative overflow-hidden bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200 p-6 mb-6 transition-all duration-200 hover:shadow-md ${className || ''}`}>
      {/* Background decoration */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-blue-300 rounded-full opacity-10 -translate-y-16 translate-x-16" />
      ...
      <div className="relative flex items-center gap-4">
        <div className="relative">
          <Avatar className="h-16 w-16 border-4 border-white shadow-lg">
            <AvatarImage src={user.photoURL || undefined} />
            <AvatarFallback className="text-xl font-bold bg-gradient-to-br from-blue-800 to-black text-white">
              {getInitials(displayName)}
            </AvatarFallback>
          </Avatar>
        ...
        <div className="flex-1 min-w-0">
          ...
          {showWelcome && (
            <p className="text-blue-700 font-medium mb-2">
              Bem-vindo ao sistema Arte Vision Agency
            </p>
          )}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex flex-col items-end text-right min-w-0">
            ...
            <div className="text-sm font-medium text-blue-700 truncate max-w-xs">
              {user.email}
            </div>
          </div>
      ...
      <div className={`mt-4 text-sm text-blue-600 font-medium italic text-center transition-opacity duration-500 ${fade ? 'opacity-0' : 'opacity-100'}`}>
