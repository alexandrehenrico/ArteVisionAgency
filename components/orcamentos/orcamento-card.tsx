...
export function OrcamentoCard({ orcamento, cliente, onStatusChange, onClick, className }: OrcamentoCardProps) {
  ...
  return (
    <Card
      className={`relative overflow-hidden bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200 shadow-sm transition-all duration-200 ${
        onClick ? "cursor-pointer hover:scale-[1.02] hover:shadow-md" : ""
      } ${className || ""}`}
      onClick={onClick}
    >
      {/* Decorações de fundo */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-blue-300 rounded-full opacity-10 -translate-y-12 translate-x-12" />
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-semibold text-lg leading-tight text-blue-900 truncate">
                {orcamento.titulo}
              </h3>
            </div>
      ...
      <CardContent className="pt-0 space-y-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="bg-white rounded-lg px-3 py-2 border border-blue-200 shadow-sm">
            <div className="text-xs text-blue-500 uppercase font-medium mb-1">Orçamento</div>
            <div className="text-sm font-bold text-blue-900">#{orcamento.numeroOrcamento}</div>
          </div>
          <div className="bg-white rounded-lg px-3 py-2 border border-blue-200 shadow-sm flex-1 min-w-0">
            <div className="text-xs text-blue-500 uppercase font-medium mb-1">Status</div>
            <Select
        ...
        <div className="bg-white rounded-lg p-3 border border-blue-200 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-blue-900">{orcamento.nomeCliente}</div>
              <div className="text-xs text-blue-500 uppercase font-medium">Cliente</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg p-3 border border-blue-200 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs text-blue-500 uppercase font-medium">Valor Total</div>
            ...
            {orcamento.desconto > 0 && (
              <div className="text-right">
                <div className="text-xs text-blue-500">Desconto</div>
        ...
        <div className="bg-white rounded-lg p-3 border border-blue-200 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="text-sm font-bold text-blue-900">Itens do Orçamento</div>
            <div className="text-xs text-blue-500">{orcamento.itens.length} item{orcamento.itens.length !== 1 ? 's' : ''}</div>
          </div>
          <div className="space-y-2">
            {orcamento.itens.slice(0, 2).map((item, index) => (
              <div key={index} className="flex items-center justify-between text-sm">
                <div className="flex-1 min-w-0 mr-3">
                  <div className="font-medium text-blue-900 truncate">{item.descricao}</div>
                </div>
            ...
            {orcamento.itens.length > 2 && (
              <div className="text-xs text-blue-500 text-center pt-2 border-t border-blue-100">
          ...
          <div className="mt-3 pt-3 border-t border-blue-100 flex justify-between">
            ...
            <span className="text-sm font-bold text-blue-900">{formatCurrency(orcamento.subtotal)}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-lg p-3 border border-blue-200 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs text-blue-500 uppercase font-medium">Criado</div>
                <div className="text-sm font-bold text-blue-900">{formatDate(orcamento.dataCriacao)}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-3 border border-blue-200 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs text-blue-500 uppercase font-medium">Válido até</div>
                <div className="text-sm font-bold text-blue-900">{formatDate(orcamento.dataVencimento)}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        {orcamento.observacoes && orcamento.observacoes.length > 50 && (
          <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
            <div className="text-xs text-blue-500 uppercase font-medium mb-2">Observações</div>
          ...
          <div className="pt-3 border-t border-blue-100">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-blue-500">
                Criado por <span className="font-bold text-blue-700">{orcamento.registradoPor}</span>
              </span>
            </div>
          </div>
        )}
        ...
        <div className="pt-2">
          <Button
            ...
            className="w-full bg-white hover:bg-blue-50 border-blue-200 hover:border-blue-300 font-medium"
            disabled={loading}
          >
            <Download className="h-4 w-4 mr-2" />
